...
pb_reg = generate_pair_based(sir_model(), net_reg;
    ...
    initial_infected = [1], tspan = (0.0, 80.0), saveat = 0.5)
...
t = range(0.0, 80.0, length = length(I_reg))
```
...
p = plot(t, I_reg, label = "Regular", lw = 2, color = :blue,
         ...
         title = "Pair-based SIR: effect of topology (N=200)")
plot!(p, t, I_er, label = "Erdős–Rényi", lw = 2, color = :orange)
plot!(p, range(0.0, 80.0, length = length(I_ba)), I_ba,
...
```
    ...
    Our τ = 0.15 → R₀ proxy above threshold: τ > τ_c = true
