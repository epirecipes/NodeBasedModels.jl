...
We run the pair-based (order-2) model on each graph with the same epidemiological parameters. On a $k$-regular graph with mean degree 6, $R_0 = (k-1)\,T$ where $T = \tau/(\tau+\gamma)$. For $R_0 = 2$: $T = 2/5$, $\tau = T\gamma/(1-T) = 1/6$.

```{julia}
T_val = 2.0 / 5.0                         # R₀ = (k-1)T = 2 for k=6
γ = 0.25
τ = T_val * γ / (1 - T_val)               # 1/6 ≈ 0.1667
...
We validate each topology against Gillespie SSA from
[`NetworkOutbreaks.jl`](https://github.com/sdwfrost/NetworkOutbreaks.jl).
For each topology, both pair-based and SSA use the same $\tau = 1/6$
($R_0 = 2$ on regular $k = 6$), $\gamma = 0.25$, and $\varepsilon = 0.05$
(5 % initial seed to suppress die-outs). Each panel shows one topology.
...
# --- Regular ---
pb_reg_v = generate_pair_based(sir_model(), net_reg;
    infection_rate = τ, recovery_rate = γ,
    tspan = (0.0, 40.0), saveat = 0.5, ε = ε_v)
I_reg_v = aggregate(pb_reg_v, :I)
t_reg_v = range(0.0, 40.0; length = length(I_reg_v))

fixed_reg = rng -> random_regular_graph(N_v, 6; seed = 123)
t_ssa_reg, μ_ssa_reg, σ_ssa_reg = gillespie_ribbon(prog_no, params_no,
    fixed_reg;
...
# --- Erdős–Rényi ---
pb_er_v = generate_pair_based(sir_model(), net_er;
    infection_rate = τ, recovery_rate = γ,
    tspan = (0.0, 40.0), saveat = 0.5, ε = ε_v)
I_er_v = aggregate(pb_er_v, :I)
t_er_v = range(0.0, 40.0; length = length(I_er_v))

t_ssa_er, μ_ssa_er, σ_ssa_er = gillespie_ribbon(prog_no, params_no,
...
# --- Barabási–Albert ---
pb_ba_v = generate_pair_based(sir_model(), net_ba;
    infection_rate = τ, recovery_rate = γ,
    tspan = (0.0, 40.0), saveat = 0.5, ε = ε_v)
I_ba_v = aggregate(pb_ba_v, :I)
t_ba_v = range(0.0, 40.0; length = length(I_ba_v))

t_ssa_ba, μ_ssa_ba, σ_ssa_ba = gillespie_ribbon(prog_no, params_no,
...
```{julia}
#| label: fig-nbm-02-validation
#| fig-cap: "Pair-based (lines) vs SSA mean ± 1σ (ribbons) for each topology (N=200, R₀=2, 5% seed). Regular and ER: PB on one graph, SSA on matching random graphs. BA: PB on one graph, SSA across 5 BA realisations."
p1 = plot(t_reg_v, I_reg_v, lw = 2, color = :blue, label = "PB",
          ylabel = "Infected", title = "Regular (k=6)")
plot!(p1, t_ssa_reg, μ_ssa_reg[:I] .* N_v, ribbon = σ_ssa_reg[:I] .* N_v,
      color = :blue, fillalpha = 0.2, linealpha = 0.4, lw = 1, label = "SSA")

p2 = plot(t_er_v, I_er_v, lw = 2, color = :orange, label = "PB",
          title = "Erdős–Rényi (⟨k⟩≈6)")
plot!(p2, t_ssa_er, μ_ssa_er[:I] .* N_v, ribbon = σ_ssa_er[:I] .* N_v,
      color = :orange, fillalpha = 0.2, linealpha = 0.4, lw = 1, label = "SSA")

p3 = plot(t_ba_v, I_ba_v, lw = 2, color = :purple, label = "PB",
          xlabel = "Time", ylabel = "Infected", title = "Barabási–Albert (m=3)")
plot!(p3, t_ssa_ba, μ_ssa_ba[:I] .* N_v, ribbon = σ_ssa_ba[:I] .* N_v,
      color = :purple, fillalpha = 0.2, linealpha = 0.4, lw = 1, label = "SSA")
