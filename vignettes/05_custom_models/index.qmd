...
The individual-based generator integrates the order-1 (NIMFA) ODE
system: each node $i$ carries probabilities $\langle S_i\rangle$,
$\langle I_i\rangle$ etc., and the infection force on $i$ is
$\sum_j \tau_{ij} \langle S_i\rangle \langle I_j\rangle$, *assuming
neighbour independence*. On a **complete graph** $K_N$ with per-edge
rate $\tau = \beta / (N - 1)$ the closure assumption is exact in the
mean-field limit, so the IB ODE collapses to standard mass-action SIR
and we can validate the IB code directly against Gillespie SSA on the
same $K_N$ host graph.

(On sparse graphs the IB ODE systematically over-predicts because
$R_0^{\text{NIMFA}} = \tau k / \gamma$ exceeds the true graph $R_0$;
that bias is a *modelling* feature of order-1 closure — see vignette 03
for the moment-closure hierarchy and vignette 07 for pair-based
corrections — not a code error.)

Setup: $N = 200$, complete graph $K_{200}$, per-edge
$\tau = 0.25 / 199$ so the population infection rate is
$\beta = \tau (N - 1) = 0.25$ and $R_0 = \beta / \gamma = 2$;
$\gamma = 0.25$; single seed $i = 1$.
...
γ_v   = 0.25
τ_e   = β_pop / (N_v - 1)
g_K   = complete_graph(N_v)
net_K = GraphNetwork(g_K)

# IB on K_N (deterministic, ≡ mass-action SIR in the mean-field limit)
sir_K = generate_individual_based(sir_model(), net_K;
    infection_rate = τ_e, recovery_rate = γ_v,
    initial_infected = [1], tspan = (0.0, 60.0), saveat = 0.5, ε = 1 / N_v)
seir_K = generate_individual_based(seir_model(), net_K;
    infection_rate = τ_e, recovery_rate = γ_v,
    initial_infected = [1], tspan = (0.0, 60.0), saveat = 0.5, ε = 1 / N_v)

I_sir_K  = aggregate(sir_K,  :I) ./ N_v
I_seir_K = aggregate(seir_K, :I) ./ N_v
t_K_sir  = range(0.0, 60.0; length = length(I_sir_K))
t_K_seir = range(0.0, 60.0; length = length(I_seir_K))
```

```{julia}
complete_builder(N) = rng -> complete_graph(N)

t_sir_no, μ_sir_no, σ_sir_no = gillespie_ribbon(sir_model(τ = :β),
    Dict(:β => τ_e, :γ => γ_v),
    complete_builder(N_v);
...
t_seir_no, μ_seir_no, σ_seir_no = gillespie_ribbon(seir_model(τ = :β),
    Dict(:β => τ_e, :γ => γ_v, :σ => 0.25),
    complete_builder(N_v);
    N = N_v, n_graphs = 1, nsims_per_graph = 200,
    tspan = (0.0, 60.0), seed_fraction = 1 / N_v,
    tgrid = collect(0.0:0.5:60.0),
...
#| fig-cap: "Validation of the order-1 individual-based ODE on a complete graph K₂₀₀ where the NIMFA closure is exact in the mean-field limit. IB curves (lines) overlap the Gillespie SSA mean ± 1σ ribbons (200 SSA replicates, conditioned on attack rate ≥ 20%) for both SIR (red) and SEIR (orange), confirming the IB code is correct. Per-edge τ = 0.25/199 → β = 0.25, γ = 0.25, R₀ = 2."
p = plot(t_K_sir, I_sir_K, label = "SIR (IB)", lw = 2, color = :red,
         xlabel = "Time", ylabel = "Infected fraction",
         title = "IB vs SSA on K₂₀₀ (mean-field limit)")
plot!(p, t_sir_no, μ_sir_no[:I], ribbon = σ_sir_no[:I],
      label = "SIR (SSA mean ± 1σ)", color = :red,
      fillalpha = 0.2, linealpha = 0.4, lw = 1)
plot!(p, t_K_seir, I_seir_K, label = "SEIR (IB)",
...
The IB curves trace the SSA ensemble mean closely on $K_N$ for both
SIR and SEIR, confirming that `generate_individual_based` correctly
implements the order-1 NIMFA system for arbitrary compartmental
models.
