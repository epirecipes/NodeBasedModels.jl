...
$\beta = 0.6$, $\gamma = 0.4$, $N = 500$, initial prevalence
$\epsilon = 0.05$, and horizon $t_{\max} = 80$.  These are the Phase C
parameters from vignette 12 and from the package testset
`"Gillespie comparison (random 3-regular, N=500)"`.
...
::: {.callout-note}
**$R_0=2$ anchor.** This homogeneous $k=3$ SIS benchmark uses the NodeBasedModels pairwise formula $R_0=\tau(k-2)/\gamma$. With $\gamma=0.25$, this gives $\tau=0.5$ and 1% initial infection.
:::
...
* Initial infected set: `collect(1:25)` (5% of `N = 500`).
