...
::: {.callout-note}
**$R_0=2$ anchor.** On a $k$-regular graph the branching-process
$R_0 = \tau(k-1)/\gamma$. With $k=6$ and $\gamma=0.25$,
$\tau = 2\gamma/(k-1) = 0.1$.
:::
...
We validate the pair-based (order-2) and individual-based (order-1)
closures against Gillespie SSA from
[`NetworkOutbreaks.jl`](https://github.com/sdwfrost/NetworkOutbreaks.jl)
on the same $k = 6$ regular graph topology ($N = 100$). We seed 5 %
of nodes ($\varepsilon = 0.05$) to suppress stochastic die-outs,
and use $\tau = R_0\gamma/(k-1) = 0.1$ ($R_0 = 2$), $\gamma = 0.25$.

```{julia}
include("../_validation.jl")

N_v = 100
τ_v = R0_target * γ_val / (k - 1)   # 0.1

# Pair-based on this graph (order-2, Kirkwood closure)
pb_val = generate_pair_based(sir_model(), net;
    infection_rate = τ_v, recovery_rate = γ_val,
    tspan = (0.0, tmax), saveat = 0.5, ε = 0.05)
I_pb_v = aggregate(pb_val, :I)
t_pb_v = range(0.0, tmax; length = length(I_pb_v))

# Individual-based on the same graph (order-1, NIMFA)
...
#| fig-cap: "Moment closure hierarchy on a k=6 regular graph (N=100, R₀=2, 5% seed). The pair-based (Kirkwood) closure matches the SSA mean; the individual-based (NIMFA) closure over-predicts due to ignoring pair correlations."
p = plot(t_pb_v, I_pb_v, label = "Pair-based (order-2)", lw = 2, color = :blue,
         xlabel = "Time", ylabel = "Number infected",
         title = "Moment closure hierarchy vs SSA (k=6, R₀=2)")
plot!(p, t_ib_v, I_ib_v, label = "Individual-based (order-1)",
...
The pair-based curve tracks the SSA mean closely; the individual-based
closure over-predicts the peak because the independence closure
$\langle S_i I_j\rangle \approx \langle S_i\rangle\langle I_j\rangle$
ignores pair correlations (see the discussion above).
