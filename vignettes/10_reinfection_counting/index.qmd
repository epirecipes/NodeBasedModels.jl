...
We use a 3-regular network of 2,000 nodes — small enough to be tractable
but large enough that local correlations dominate, which is precisely the
regime where standard pairwise fails most visibly.

```{julia}
n_nodes  = 2_000
k        = 3
τ, γ     = 0.6, 1.0          # R₀_pairwise = τ(k-1)/γ ≈ 1.2 ⇒ endemic
tspan    = (0.0, 60.0)
...
groupedbar(0:4, hcat(ode_S, sim_S, ode_I, sim_I);
           label = ["ODE S_p" "SSA S_p" "ODE I_p" "SSA I_p"],
           xlabel = "infection count p (capped at $4)",
           ylabel = "population fraction",
           title  = "Reinfection-count histogram at t = 60",
           bar_position = :dodge)
```
